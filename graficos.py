...
def gerar_grafico_acoes(carteira, num):
    ...
    histacoes1 = histacoes.loc[(histacoes["symbol"] == f'{lista_acoes[0]}'), [
        "date", "close"]]
    histacoes2 = histacoes.loc[(histacoes["symbol"] == f'{lista_acoes[1]}'), [
        "date", "close"]]
    histacoes3 = histacoes.loc[(histacoes["symbol"] == f'{lista_acoes[2]}'), [
        "date", "close"]]
    if len(lista_acoes) >= 4:
        histacoes4 = histacoes.loc[(histacoes["symbol"] == f'{lista_acoes[3]}'), [
            "date", "close"]]
    else:
        histacoes4 = []
    if len(lista_acoes) >= 5:
        histacoes5 = histacoes.loc[(histacoes["symbol"] == f'{lista_acoes[4]}'), [
            "date", "close"]]
    else:
        histacoes5 = []
    if len(lista_acoes) >= 6:
        histacoes6 = histacoes.loc[(histacoes["symbol"] == f'{lista_acoes[5]}'), [
            "date", "close"]]
    else:
        histacoes6 = []
    if len(lista_acoes) >= 7:
        histacoes7 = histacoes.loc[(histacoes["symbol"] == f'{lista_acoes[6]}'), [
            "date", "close"]]
    else:
        histacoes7 = []
...
def gerar_grafico_moedas(carteira, num):
    ...
    if len(lista_moedas) >= 5:
        histmoedas5 = histmoedas.loc[(
            histmoedas["symbol"] == f'{lista_moedas[4]}BRL=X'), ["date", "close"]]
    else:
        histmoedas5 = []
    ...
    if len(lista_moedas) >= 5:
        plt.plot(histmoedas5.date, histmoedas5.close,
                 label=f'{lista_moedas[4]}')
...
def gerar_grafico_carteira(carteira, num):
    ...
    # Gerando o plot
    plt.plot(histcarteira.date, histcarteira.open,
             label="Valor total da carteira em Reais")
    plt.legend()
